...
function buildContextData() {
    ...
    // ✅ FIX: hariIni, besok, tanggal selalu dihitung ulang (tidak ikut cache)
    const nowJkt  = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Jakarta" }));  // UBAH
    const besokJkt = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Jakarta" })); // UBAH
    besokJkt.setDate(besokJkt.getDate() + 1);                                                    // UBAH

    const hariIni = getNamaHari(nowJkt);   // UBAH
    const besok   = getNamaHari(besokJkt); // UBAH
    const tanggal = getTanggalFormatted(nowJkt); // UBAH

    if (cachedContext && (now - cacheTimestamp) < CACHE_TTL_MS) {
        // Cache hanya untuk PR/jadwal, tapi return dengan tanggal fresh
        return { ...cachedContext, hariIni, besok, tanggal }; // UBAH
    }
